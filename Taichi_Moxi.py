...
class Taichi_Moxi:
    ...
    def __init__(self, res=512) -> None:
        ...
        self.s0 = ti.root
        self.s1 = self.s0.pointer(ti.ij, 32)
        self.s2 = self.s1.dense(ti.ij, 8)
        self.s3 = self.s2.dense(ti.ij, 2)
        # self.s3.place(self.Pigment_flow_c)
        # self.s3.place(self.Pigment_flow_a)
        # self.s3.place(self.psy)
        # self.s3.place(self.Pigment_flow_star_c)
        # self.s3.place(self.Pigment_flow_star_a)
        self.s3.place(self.Flow)
        # self.s3.place(self.FlowNext)
        # self.s3.place(self.rou)
    ...
    def drawStrok(self):
        ...
        for i, j in ti.ndrange(self.res, self.res):
            ...
            if dis < self.brushRadius:
                ...
                # max(1-self.rou[i,j]/0.5, 0.7)
                self.Water_Surface[i, j] += 0.7
    ...
    def waterSurface_to_flow(self):
        for P in ti.grouped(self.Water_Surface):
            if self.Water_Surface[P] > 0:
                ...
                #     self.Water_Surface[P], 0, 1.0-self.rou[P])
        ...
        for P in ti.grouped(self.psy):
            if (self.psy[P] > 0):
                ...
                self.Pigment_flow_c[P] = (self.Pigment_flow_c[P]*self.rou[P]*self.Pigment_flow_a[P] +
                    ...
                    (self.rou[P]*self.Pigment_flow_a[P] +
    ...
    def update_Pf(self):
        for P in ti.grouped(self.Pigment_flow_c):
            gama_star = tg.scalar.mix(1, 0.1, tg.scalar.smoothstep(
                self.FlowVelocity[P].norm()*self.pigment_flow_rate[None], 0, 0.05))
    ...
    @ti.kernel
    def render(self):
        ti.activate(self.s1,[12,12])
        for i, j in ti.ndrange(self.res, self.res):
            s_index = ti.rescale_index(self.Flow, self.s1, [i, j])
            if (ti.is_active(self.s1, s_index)):
                self.FrameBuffer[i, j] = ti.Vector([0.0, 0.0, 0.0])
            else:
                self.FrameBuffer[i, j] = ti.Vector([1.0, 1.0, 1.0])
            ...
            # self.FrameBuffer[i,j] = tg.scalar.mix(
            #     self.FrameBuffer[i,j], ti.Vector([0.0, 0.2, 0.6]), self.rou[i,j]*0.5)
